...
export class AsyncCreatable<TValue = OptionValues> extends React.Component<ReactAsyncCreatableSelectProps<TValue>, any> { }
...
export type FilterOptionsHandler<TValue = OptionValues> = (options: Options<TValue>, filter: string, currentValues: Options<TValue>, props: ReactSelectProps) => Options<TValue>;
...
export type OnBlurHandler = (event: React.FocusEvent<HTMLDivElement | HTMLInputElement>) => boolean;
export type OptionRendererHandler<TValue = OptionValues> = (option: Option<TValue>, index?: number, inputValue?: string) => HandlerRendererResult;
...
export function defaultMenuRenderer<TValue = OptionValues>(props: MenuRendererProps<TValue>): HandlerRendererResult;
export function defaultArrowRenderer(props: ArrowRendererProps): HandlerRendererResult;
export function defaultClearRenderer(): HandlerRendererResult;
export function defaultFilterOptions<TValue = OptionValues>(options: Options<TValue>, filter: string, currentValues: Options<TValue>, props: ReactSelectProps): Options<TValue>;
...
export interface MenuRendererProps<TValue = OptionValues> {
    ...
    /**
     * Callback to be invoked when an option is focused.
     */
    onFocus: (option: Option<TValue>, event: any) => void;

    onOptionRef: (option: Option<TValue>, isFocused: boolean) => void;
    ...
    /**
     * Custom class name for the options
     */
    ...
    /**
     * Shows check boxes next to options
     */
    ...
    /**
     * Option values are accessible with this string key.
     */
    valueKey: string;
}
...
export interface ReactSelectProps<TValue = OptionValues> extends React.Props<ReactSelectClass<TValue>> {
    ...
    searchable?: boolean;
    /**
     * Whether the dropdowns should always be expanded or not.
     */
    shouldAlwaysBeOpen?: boolean;
    /**
     * Shows checkboxes in multi-select dropdown if true. Also causes options to not disappear if selected.
     */
    showCheckbox?: boolean;
    /**
     * Shows tags if true, "X selected" if false
     */
    showTags?: boolean;
    /**
