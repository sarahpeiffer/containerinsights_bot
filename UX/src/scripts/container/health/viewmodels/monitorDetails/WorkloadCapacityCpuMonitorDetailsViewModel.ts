...
import { WorkloadCapacityCpuMonitorDetailsModel } from '../../models/monitorDetails/WorkloadCapacityCpuMonitorDetailsModel';
...
import { IWorkloadCapacityCpuMetricDetails, IWorkloadCapacityCpuDetails, IUsageDataObj } from './MonitorDetailsTypings';
import { HealthState } from '../../HealthState';
import moment = require('moment');

/**
 * View model for workload CPU capacity monitor details component 
*/
export class WorkloadCapacityCpuMonitorDetailsViewModel extends HealthMonitorDetailsViewModelBase  {
    private _model: WorkloadCapacityCpuMonitorDetailsModel;
    ...
    */
    ...
    public convertModelIntoSGDataRows(): SGDataRow[] {
        const metrics: IWorkloadCapacityCpuMetricDetails = this._model.details.details;
        const limit: number = metrics.clusterCpuCapacity;
        const request: number = metrics.clusterCpuRequests;
        ...
        const status = request < limit ? HealthState.Healthy : HealthState.Critical;
    ...
    protected createDetailsModel(monitorIdentifier: string): void {                   
        if (!monitorIdentifier) { 
            throw new Error(`@monitorIdentifier may not be null at WorkloadCapacityCpuMonitorDetailsViewModel.createDetailsModel()`); 
        }
        ...
        const details: IWorkloadCapacityCpuDetails = this.getDetails(monitor); // extract details
        const sortColumn = 0;
        const sortOrder = SGSortOrder.Ascending;

        this._model = new WorkloadCapacityCpuMonitorDetailsModel(
            details, 
            sortColumn, 
            sortOrder,
     ...
     * @param monitor
     */
    private getDetails(monitor: IHealthMonitor): IWorkloadCapacityCpuDetails {
        if (!monitor) { 
            throw new Error('@monitor may not be null in WorkloadCapacityCpuMonitorDetailsViewModel.getDetails()'); 
        }

        let details: IWorkloadCapacityCpuDetails = monitor.details;

        if (!details) {
            throw new Error('@details may not be null in WorkloadCapacityCpuMonitorDetailsViewModel.getDetails()');
        }
